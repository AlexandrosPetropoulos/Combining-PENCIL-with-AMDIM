...
def test_model_2(model, test_loader, device, stats, max_evals=200000):
    '''
    Evaluate accuracy on test set
    '''
    # warm up batchnorm stats based on current model
    _warmup_batchnorm_2(model, test_loader, device, batches=50, train_loader=False)
    ...
    linear_y_file_logit = os.path.join(current_dir,"linear_y_logit.npy")
    mlp_y_file_logit = os.path.join(current_dir,"mlp_y_logit.npy")

    linear_y_logit = np.zeros([50000,10])
    mlp_y_logit = np.zeros([50000,10])
    ...
    correct_glb_lin = 0.
    total = 0.
    for _, (images, labels, index) in enumerate(test_loader):
        if total > max_evals:
            break
        ...
        with torch.no_grad():
            ...
            linear_y_logit[index, :] = lgt_glb_lin.to(torch.device("cpu")).numpy()
            mlp_y_logit[index, :] = lgt_glb_mlp.to(torch.device("cpu")).numpy()
    ...
    np.save(linear_y_file_logit,linear_y_logit)
    np.save(mlp_y_file_logit,mlp_y_logit)
...
def _warmup_batchnorm_2(model, data_loader, device, batches=100, train_loader=False):
    '''
    Run some batches through all parts of the model to warmup the running
    stats for batchnorm layers.
    '''
